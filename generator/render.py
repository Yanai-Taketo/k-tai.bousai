...
def page(title, body, generated, root="./", banner=""):
    ...
    return (
        ...
        f"<div class=hd>データ取得: {e(generated)} JST | 気象庁発表の軽量ミラー(個人運営)</div>"
        f"{warn}{body}"
        # 免責は1行に圧縮し全文はaboutへ委譲する(津波ページでは本体より注意書きが
        # 長いという本末転倒を解消)。「非公式の転載」という書き方は情報そのものが
        # 疑わしく読めるため、①出典は気象庁の発表そのまま ②運営は気象庁でなく個人、
        # の順に述べる。R-5(非公式サイトである旨の明示)はこの②とヘッダで満たす。
        f'<p class=n>本サイトは気象庁の公式サイトではなく、個人が運営しています。'
        f'避難や身を守る行動の判断は、必ずお住まいの自治体・気象庁の発表に'
        f'従ってください。<a href="{root}about">詳しく</a></p>'
...
def render_about(generated):
    body = (
        ...
        "<h2>ご注意(免責)</h2>"
        "<p>掲載している情報は、すべて<strong>気象庁が発表した内容をそのまま</strong>"
        "掲載したものです(発表主体・発表時刻は各項目に記載のとおり)。内容を独自に"
        "加工・予測することはありません。</p>"
        "<p>ただし<strong>本サイトは気象庁の公式サイトではなく、個人が運営しています</strong>。"
        "取得や表示の過程で遅延・欠落が生じる可能性があるため、情報の即時性・完全性は"
        "保証できません。"
